...
class Sweepmine_Gaming:
    def __init__(self, owner):
        self.owner = owner
        self.owner.write_name()
        self.open_num = 0
        self.board = [['' for i in range(self.owner.length)] for i in range(self.owner.width)]
        self.icon_board = [[[] for i in range(self.owner.length)] for i in range(self.owner.width)]
        self.start_pos = tuple(
            map(lambda x, y: (x - y * self.owner.depth) // 2, self.owner.screen_rect, self.owner.board_rect))
        self.__update_mine()
        self.total_draw()

    def __update_mine(self):
        self.mine_list = []
        for i in range(self.owner.mine_num):
            while True:
                mine_pos = (random.randint(0, self.owner.length - 1), random.randint(0, self.owner.width - 1))
                if mine_pos not in self.mine_list:
    ...
    def unfold_function(self, x, y):
        if 0 <= x < self.owner.length and 0 <= y < self.owner.width and isinstance(self.board[y][x], str):
            num = 0
            self.open_num += 1
            ...
            pygame.draw.rect(self.owner.screen, (150, 150, 150), (
                ...
                self.owner.depth - 1))
            if num:
                self.extend_icon(x, y, str(num))
            else:
                self.chain_unfold(x, y)
    ...
    def chain_unfold(self, x, y):
        for i in (-1, 0, 1):
            for j in (-1, 0, 1):
                if 0 <= x + i < self.owner.length and 0 <= y + j < self.owner.width and isinstance(
                        self.board[y + j][x + i], str):
                    self.unfold_function(x=x + i, y=y + j)
...
class Sweepmine_Main:
    def __init__(self):
        ...
        sys.setrecursionlimit(100000)  # 待改进
    ...
    def loop(self):
        starting_page = Sweepmine_Starting(self)
        running = True
        while running:
                ...
                if self.state == self.state_list[0]:
                    if event.type == pygame.MOUSEBUTTONUP:
                        if pygame.Rect.collidepoint(starting_page.font_list[1][1], event.pos):
                            ...
                            gaming_page = Sweepmine_Gaming(self)
                            del starting_page
                            break
                    elif event.type == pygame.KEYDOWN:
                        if event.key == pygame.K_ESCAPE:
                            self.state = self.state_list[2]
                            setting_page = Sweepmine_Setting(self)
                            del starting_page
